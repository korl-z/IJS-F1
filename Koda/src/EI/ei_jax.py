...
# @dataclass(frozen=True)
# class PhononBath:
#     """One longitudinal acoustic branch on the supplied electron momentum grid.

#     k has shape (bd.size, 2) and must follow the order of bd.ea and bd.eb.
#     Energies use hbar = k_B = 1; cs converts reciprocal length to energy.
#     Only normal processes with |q_x|, |q_y| <= qmax are included.
#     """
...
#             raise ValueError("k must be a finite array of shape (bd.size, 2)")
#         if not all(np.isrealobj(x) for x in (
#             self.t, self.cs, self.ktf, self.eta, self.amp,
#             self.ca, self.cb, self.qmax,
#         )):
#             raise ValueError("phonon parameters and orbital weights must be real")
#         if not all(np.isfinite(x) for x in (
#             self.t, self.cs, self.ktf, self.eta, self.amp,
#             self.ca, self.cb, self.qmax,
#         )):
#             raise ValueError("phonon parameters must be finite")
#         if min(self.t, self.cs, self.ktf, self.eta, self.qmax) <= 0 or self.amp < 0:
#             raise ValueError("t, cs, ktf, eta, qmax must be positive; amp nonnegative")
#         object.__setattr__(self, "k", tuple(map(tuple, k)))


# def gam_ph(t: float, k: Any, cs: float, ktf: float, eta: float,
#            amp: float = 1.0, ca: float = 1.0, cb: float = 1.0,
#            qmax: float = np.pi, name: str = "phonon") -> PhononBath:
#     """Make a screened, one-branch electron-phonon bath."""
#     return PhononBath(t, k, cs, ktf, eta, amp, ca, cb, qmax, name)
...
# def _baths(bs: Iterable[Bath | PhononBath]) -> tuple[tuple[Any, ...], tuple[RateFn, ...]]:
#     bs = tuple(bs)
#     ls = tuple(b for b in bs if isinstance(b, Bath))
#     ps = tuple(b for b in bs if isinstance(b, PhononBath))
#     if not bs or len(ls) + len(ps) != len(bs):
#         raise ValueError("bs must contain Bath or PhononBath objects")

#     oc = {None: 0, "a": 1, "b": 2}
#     bt = jnp.asarray([b.t for b in ls], dtype=F32)
#     bk = jnp.asarray([_kap_mat(b.kap) for b in ls], dtype=F32).reshape(-1, 2, 2)
#     bw = jnp.asarray([b.wc for b in ls], dtype=F32)
#     bo = jnp.asarray([oc[b.orb] for b in ls], dtype=I32)
#     bm = jnp.asarray([b.mu for b in ls], dtype=F32)
#     pk = (jnp.asarray([b.k for b in ps], dtype=F32)
#           if ps else jnp.zeros((0, 0, 2), dtype=F32))
#     pp = (
#         pk,
#         jnp.asarray([b.t for b in ps], dtype=F32),
#         jnp.asarray([b.cs for b in ps], dtype=F32),
#         jnp.asarray([b.ktf for b in ps], dtype=F32),
#         jnp.asarray([b.eta for b in ps], dtype=F32),
#         jnp.asarray([b.amp for b in ps], dtype=F32),
#         jnp.asarray([b.ca for b in ps], dtype=F32),
#         jnp.asarray([b.cb for b in ps], dtype=F32),
#         jnp.asarray([b.qmax for b in ps], dtype=F32),
#     )
#     rf = tuple(b.rate for b in ls)
#     return (bt, bk, bw, bo, bm, pp), rf
...
# def _g_long(q: Any, cs: Any, ktf: Any, amp: Any) -> tuple[Any, Any]:
#     """Use an even longitudinal polarization so g(-q) = conj(g(q))."""
#     q2 = jnp.sum(q * q, axis=-1)
#     qm = jnp.sqrt(q2)
#     sg = jnp.where(jnp.abs(q[..., 0]) > F32(1.0e-7),
#                    jnp.sign(q[..., 0]), jnp.sign(q[..., 1]))
#     om = cs * qm
#     om0 = jnp.where(qm > 0, om, F32(1.0))
#     g = -1j * amp * sg * qm / (jnp.sqrt(om0) * (q2 + ktf * ktf))
#     return g, om


# def _phon_rates(st: JState, pp: tuple[Any, ...]) -> Any:
#     pk, pt, pc, ps, pe, pa, pca, pcb, pq = pp
#     e = st.e.reshape(-1)
#     de = e[:, None] - e[None, :]
#     ua, ub = _qv(st)
#     r = jnp.zeros_like(de)
...
#         k = jnp.tile(pk[i], (2, 1))
#         q = k[:, None, :] - k[None, :, :]
#         g, om = _g_long(q, pc[i], ps[i], pa[i])
#         om0 = jnp.where(om > 0, om, F32(1.0))
#         nqp = F32(1.0) / jnp.expm1(om0 / pt[i])
#         nqm = nqp  # The acoustic branch has omega(-q) = omega(q).
#         af = (pca[i] * ua[:, None] * ua[None, :]
#               + pcb[i] * ub[:, None] * ub[None, :])
#         zm = (de - om) / pe[i]
#         zp = (de + om) / pe[i]
#         fac = F32(1.0) / (jnp.sqrt(F32(2.0 * np.pi)) * pe[i])
#         shell = fac * (nqp * jnp.exp(-F32(0.5) * zm * zm)
#                        + (F32(1.0) + nqm) * jnp.exp(-F32(0.5) * zp * zp))
#         valid = ((om > 0) & (jnp.abs(q[..., 0]) <= pq[i])
#                  & (jnp.abs(q[..., 1]) <= pq[i]))
#         r = r + jnp.where(valid, F32(2.0 * np.pi) * jnp.abs(g * af)**2 * shell,
#                           F32(0.0))
...
# def dense_rates(st: JState, bs: Iterable[Bath | PhononBath]) -> Any:
#     """Build the summed dense rate matrix on the JAX device."""
#     bp, rf = _baths(bs)
#     return _rates_jit(st, bp, rf)
...
# def dense_current(bd: eu.Bands, st: JState,
#                   n: Any, bs: Iterable[Bath | PhononBath]) -> Any:
#     """Evaluate the current using a full rate matrix."""
#     return total_current(bd, st, n, bs, mode="dense")
...
# def _check_ph(bs: tuple[Bath | PhononBath, ...], bd: eu.Bands,
#               mode: str) -> None:
#     for b in bs:
#         if isinstance(b, PhononBath):
#             if len(b.k) != bd.size:
#                 raise ValueError("phonon k must match bd.size and bd momentum order")
#             if mode != "dense":
#                 raise ValueError("phonon baths currently require mode='dense'")
